...
const animationTimeline = () => {
    ...
    // -----------------------------
    // CẬP NHẬT BÓNG BAY NGẪU NHIÊN
    const balloons = document.querySelectorAll(".baloons img");

    balloons.forEach((img, index) => {
        // chia hai bên: index chẵn → trái, index lẻ → phải
        if (index % 2 === 0) {
            img.style.left = Math.random() * 20 + "%"; // bên trái
        } else {
            img.style.left = 80 + Math.random() * 20 + "%"; // bên phải
        }
    
        // scale ngẫu nhiên 0.5 → 1.2
        const scale = 0.5 + Math.random() * 0.7;
        img.style.transform = "scale(" + scale + ")";
    
        // opacity ngẫu nhiên 0.5 → 1
    ...
    tl.staggerFromTo(
        ...
        2.5,
        { opacity: 0.9, y: 1400 },
        { opacity: 1, y: -1000, ease: Linear.easeNone, repeat: -1 },
        0.5
    )
